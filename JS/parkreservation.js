...
document.addEventListener("DOMContentLoaded", function () {
  reservationModal = new bootstrap.Modal(
...
function applyFilter(filterType) {
  ...
  if (filterType === "general") {
    filteredList = establishments.filter(
      (e) => e.type === "Parking Lot" || e.type === "Government Building"
    );
...
document
  ...
  .addEventListener("click", () => applyFilter("general"));
document
  .getElementById("reservedBtn")
  .addEventListener("click", () => applyFilter("reserved"));
...
function renderEstablishments(establishmentsToRender = establishments) {
  ...
  container.innerHTML = establishmentsToRender
    .map((establishment) => {
      ...
      const occupiedSpots = hasKnownAvailability
        ? establishment.totalSpots - availableNum
        : "N/A";

      const isFullyBooked = hasKnownAvailability ? availableNum === 0 : false;
      const isLimited = hasKnownAvailability
        ? availableNum <= 10 && availableNum > 0
        : false;

      const isGeneralParking =
        establishment.type === "Parking Lot" ||
        establishment.type === "Government Building";
...
function filterEstablishments() {
  ...
  const filtered = establishments.filter((establishment) => {
    ...
    if (availabilityFilter === "available") {
      ...
      matchesAvailability =
        establishment.availableSpots > 0 && establishment.availableSpots <= 10;
    }
    ...
    if (currentParkingFilter === "general") {
      ...
      matchesParkingType =
        ...
        establishment.type === "Integrated Resort";
    }
...
function confirmReservation() {
  ...
  const serviceFee = (parkingFee + roomFee) * 0.1;
  const total = parkingFee + serviceFee + roomFee;

  document.getElementById("chargesEstablishment").textContent =
    currentEstablishment.name;
...
function finalizeReservation() {
  ...
  currentEstablishment.availableSpots = Math.max(
    0,
    currentEstablishment.availableSpots - 1
  );
  ...
  const alertHtml = `
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            ...
            <p class="mb-0">
                ...
                <strong>Date:</strong> ${reservation.arrivalDate} at ${
    reservation.arrivalTime
  }<br>
                <strong>Duration:</strong> ${reservation.duration} hours<br>
                <strong>Total Cost:</strong> ₱${reservation.totalCost.toFixed(
                  2
                )}<br>
...
document
  ...
  .addEventListener("input", filterEstablishments);
document
  .getElementById("availabilityFilter")
  .addEventListener("change", filterEstablishments);
